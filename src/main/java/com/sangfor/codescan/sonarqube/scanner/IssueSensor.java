...
public class IssueSensor implements Sensor {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    ...
    return fs.hasFiles(fs.predicates().hasLanguage("c++"));
  }

  @Override
  public void analyse(Project project, SensorContext sensorContext) {
    // This sensor create an issue on each java file
    for (InputFile inputFile : fs.inputFiles(fs.predicates().hasLanguage("c++"))) {
      Issuable issuable = perspectives.as(Issuable.class, inputFile);
      issuable.addIssue(issuable.newIssueBuilder()
        .ruleKey(RuleKey.of("cpptest", "BD-MISC-TRANS"))
        .message("资源没有释放，，这个只是一个demo ，，不要当真哦 。。。")
        .line(1)
        .build());
    }
  }
